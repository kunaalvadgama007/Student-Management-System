...
class Student:
	def __init__(self,root):
		...
		headinglbl=Label(root,text="Student Management System",font=("arial",24,"bold"),bg='blue',fg='red')
		headinglbl.pack(side=TOP,fill=X)

		#Frame-1
		entry_frame=Frame(root,bd=5,relief='ridge',bg='blue')
		entry_frame.place(x=20,y=50,width=350,height=745)



		#Labels of frame-1
		reg_lbl=Label(entry_frame,text="Registration Form",font=("arial",20,"bold"),bg='blue',fg='red')
		...
		Fname_lbl=Label(entry_frame,text="Last name",font=("",13))
		...
		fname_entry=Entry(entry_frame,bd=3,relief='ridge',font=("",12),textvariable=self.last_name)
		...
		branch_combo['values']=('Computer Science','Information Technology','Data Science','Mechanical','Electronics','Artificial Intelligence','Machine Learning')
		...
		def add_data():
			if self.roll_no.get()=="" or self.name.get()==""  or self.last_name.get()=="" or self.contact.get()=="" or self.add_txt.get('1.0',END)=="" or len(self.contact.get())!=10:
				...
				cur=con.cursor()
				cur.execute('insert into students values(%s,%s,%s,%s,%s,%s,%s,%s,%s)',(self.roll_no.get(),self.name.get(),self.last_name.get(),self.gen.get(),self.category.get(),self.branch.get(),self.year.get(),self.contact.get(),self.add_txt.get('1.0',END)))
				con.commit()
				con.close()	
				fetch_data()
				clear_data()
		...
		def update_data():
			if self.roll_no.get()=="" or self.name.get()==""  or self.last_name.get()=="" or self.contact.get()=="" or self.add_txt.get('1.0',END)=="" or len(self.contact.get())!=10:
				...
				con=pymysql.connect(host='localhost',user='root',password='',database='student_management_system')
				cur=con.cursor()
				cur.execute('update students set Name=%s , Last_Name=%s , Gender=%s , Category=%s , Branch=%s , Year=%s , Contact_no=%s , Address=%s where Roll_no=%s ',(self.name.get(),self.last_name.get(),self.gen.get(),self.category.get(),self.branch.get(),self.year.get(),self.contact.get(),self.add_txt.get('1.0',END),self.roll_no.get()))
				con.commit()
				con.close()
				fetch_data()
				clear_data()
		...
		btn_frame=Frame(entry_frame,bd=5,relief='ridge',bg='blue')
		...
		#Frame-2
		data_frame=Frame(root,bd=5,relief='ridge',bg='light blue')
		...
		view_frame=Frame(data_frame,bd=5,relief='ridge',bg='light blue')
		...
		table=ttk.Treeview(view_frame,columns=('Roll_no.','Name','Last_Name','Gender','Category','Branch','Year','Contact_no.','Address'),xscrollcommand=x_scroll.set,yscrollcommand=y_scroll.set)
		...
		table.column("Roll_no.",width=30)
		table.column("Name",width=80)
		table.column("Last_Name",width=80)
